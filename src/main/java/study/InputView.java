...
public class InputView {
    ...
    public static boolean checkCoordinateQuad(int[] output) {
        double averageX = (double) (output[0] + output[2] + output[4] + output[6]) / 4;
        double averageY = (double) (output[1] + output[3] + output[5] + output[7]) / 4;
        double expectedX = Math.abs(output[0] - averageX);
        double expectedY = Math.abs(output[1] - averageY);
        for (int i = 2; i < 8; i += 2) {
            if(expectedX != Math.abs(output[i] - averageX)) return false;
