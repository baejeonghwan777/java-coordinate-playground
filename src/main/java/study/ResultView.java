...
public class ResultView {
    ...
    public static StringBuilder printEqualPointDecide(Coordinates coordinates, int max, int index) {
        ...
        if (coordinates.getValue("firstX") < coordinates.getValue("secondX")) {
            ...
            if (coordinates.getValue("secondY") == max - index)
                output.append(printEqualPoint(coordinates, "secondX", "firstX"));
        }
        if (coordinates.getValue("firstX") >= coordinates.getValue("secondX")) {
            if (coordinates.getValue("secondY") == max - index) output.append(printPoint(coordinates, "secondX"));
            if (coordinates.getValue("firstY") == max - index)
                output.append(printEqualPoint(coordinates, "firstX", "secondX"));
        }
        return output;
    }
    ...
    public static StringBuilder analyzePointLine(Line coordinates, int max, int index) {
        StringBuilder output = new StringBuilder();
        if (coordinates.equalPoint())
            if (coordinates.getValue("firstY") == max - index) return output.append(printPoint(coordinates, "firstX"));
